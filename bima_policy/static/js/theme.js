...
var Theme = /*#__PURE__*/function () {
  function Theme() {
    ...
    // SG color scheme refer from our _variable-bs-overrides.scss
    this.colors = {
      black: '#14141F',
      brand: {
        blue: '#0179A8',
 ...
 */
 ...
 if($('button.hamburger').is(':visible')){
   $("div#navbarTogglerDemo01 a.nav-link").click(function(){
      $("button.hamburger").trigger('click'); 
   }); 
 }   

var SG = function () {
  var SG = new Theme(); // toggle skin thought button

  $('[data-toggle="skin"]').on('click', function (e) {
    e.preventDefault();
    var skin = SG.skin === 'dark' ? 'default' : 'dark';
    SG.setSkin(skin); // we need to refresh our page after change the skin

    location.reload();
  }).each(function () {
    var isDarkSkin = SG.skin === 'dark';
